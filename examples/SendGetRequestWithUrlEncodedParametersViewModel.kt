...
class SendGetRequestWithUrlEncodedParametersViewModel : ViewModel() {
    ...
    internal var getQueryParamEncodedResponse = MutableLiveData<String>()
       ...
       try {
            viewModelScope.launch(Dispatchers.Main) {
                val result: Result<WeatherResponse, HoodiesNetworkError> = withContext(Dispatchers.IO) {
                    ...
                    weatherHttpClient.getUrlQueryParamEncoded(
                ...
                when (result) {
                    is Success -> {
                        getQueryParamEncodedResponse.postValue(gson.toJson(result.value))
                    }
                    is Failure -> {
                        getQueryParamEncodedResponse.postValue(result.reason.message)
                    }
                }
            }
