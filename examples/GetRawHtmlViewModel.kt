...
class GetRawHtmlViewModel : ViewModel() {
    ...
    internal fun sendGetHtmlRequest() {
        viewModelScope.launch(Dispatchers.Main) {
            ...
            when (result) {
                is Success -> {
                    getRawHtmlResponse.postValue(result.value)
                }
                is Failure -> {
                    getRawHtmlResponse.postValue(getError(result.reason))
                }
            }
        }
    }
